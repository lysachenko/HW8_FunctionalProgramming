...
public class Executor {
    ...
    private long getCountOfUsersNotLiveInCity(List<User> users, String city) {
        return users.stream()
            .filter(user -> !user.getCity().equals(city))
            .count();
    }

    private long getAverageAgeOfUsersInCity(List<User> users, String city) {
        return getSumOfAge(users, city) / getCountOfUsers(users, city);
    }

    private Integer getMinValue(List<Integer> integerList) {
        return integerList.stream().min(Integer::compare).get();
    }

    private Integer getMaxValue(List<Integer> integerList) {
        return integerList.stream().max(Integer::compare).get();
    }

    private void showValuesIncreasedByTen(List<Integer> integerList) {
        integerList.stream()
    ...
    private void showValueMultipleOfTwo(List<Integer> integerList) {
        integerList.stream()
            .filter(integer -> integer % 2 == 0)
            .forEach(integer -> System.out.print(integer + " "));
    }

    private void showSortUsersLimitThree(List<User> users) {
        users.stream()
    ...
    private int getSumOfAge(List<User> users, String city) {
        return users.stream()
    ...
    private long getCountOfUsers(List<User> users, String city) {
        return users.stream()
            .filter(user1 -> user1.getCity().equals(city))
            .count();
    }

    private void showUsersUnderFiftyAndLiveInDnipro(List<User> users, String city) {
        users.stream()
    ...
    private void showUsersOverForty(List<User> users) {
        users.stream()
